...
__export(dist_exports, {
  ...
  _PointLabelLayer: () => point_label_layer_default,
...
var VERSION = true ? "9.0.38" : "latest";
...
var VERSION2 = true ? "9.0.38" : "latest";
...
function parseCartoRasterTile(arrayBuffer, options) {
  if (!arrayBuffer)
    return null;
  const { bands, blockSize } = parsePbf(arrayBuffer, TileReader3);
  const numericProps = {};
...
var fs = `uniform heatmapUniforms {
vec2 colorDomain;
vec2 delta;
float intensity;
float opacity;
...
var fs2 = `vec4 copy_filterColor(vec4 color, vec2 texSize, vec2 texCoord) {
  return color;
}
`;
var copy = {
...
var VERSION3 = true ? "9.0.38" : "latest";
...
var DEFAULT_PARAMETERS = {
  v: V3_MINOR_VERSION,
  deckglVersion: import_core16.VERSION
};
var DEFAULT_HEADERS = {
...
async function requestWithParameters({ baseUrl, parameters = {}, headers: customHeaders = {}, errorContext, maxLengthURL = DEFAULT_MAX_LENGTH_URL }) {
  ...
  const fetchPromise = url.length > maxLengthURL ? fetch(baseUrl, { method: "POST", body: JSON.stringify(parameters), headers }) : fetch(url, { headers });
  let response;
  let responseJson;
...
var h3QuerySource = async function(options) {
  const { aggregationExp, aggregationResLevel = DEFAULT_AGGREGATION_RES_LEVEL_H3, sqlQuery, spatialDataColumn = "h3", queryParameters, filters } = options;
  const urlParameters = {
    aggregationExp,
    spatialDataColumn,
    spatialDataType: "h3",
...
var h3TableSource = async function(options) {
  const { aggregationExp, aggregationResLevel = DEFAULT_AGGREGATION_RES_LEVEL_H3, spatialDataColumn = "h3", tableName, filters } = options;
  const urlParameters = {
    aggregationExp,
    name: tableName,
    spatialDataColumn,
...
var quadbinQuerySource = async function(options) {
  const { aggregationExp, aggregationResLevel = DEFAULT_AGGREGATION_RES_LEVEL_QUADBIN, sqlQuery, spatialDataColumn = "quadbin", queryParameters, filters } = options;
  const urlParameters = {
    aggregationExp,
    q: sqlQuery,
    spatialDataColumn,
...
async function _fetchMapDataset(dataset, accessToken, apiBaseUrl, clientId, headers, maxLengthURL = DEFAULT_MAX_LENGTH_URL) {
...
async function _fetchTilestats(attribute, dataset, accessToken, apiBaseUrl, maxLengthURL = DEFAULT_MAX_LENGTH_URL) {
...
async function fillInMapDatasets({ datasets, token }, clientId, apiBaseUrl, headers, maxLengthURL = DEFAULT_MAX_LENGTH_URL) {
  const promises = datasets.map((dataset) => _fetchMapDataset(dataset, token, apiBaseUrl, clientId, headers, maxLengthURL));
  return await Promise.all(promises);
}
async function fillInTileStats({ datasets, keplerMapConfig, token }, apiBaseUrl, maxLengthURL = DEFAULT_MAX_LENGTH_URL) {
  var _a;
  const attributes = [];
  ...
  const promises = filteredAttributes.map(({ attribute, dataset }) => _fetchTilestats(attribute, dataset, token, apiBaseUrl, maxLengthURL));
  return await Promise.all(promises);
}
async function fetchMap({ accessToken, apiBaseUrl = DEFAULT_API_BASE_URL, cartoMapId, clientId = DEFAULT_CLIENT, headers = {}, autoRefresh, onNewData, maxLengthURL = DEFAULT_MAX_LENGTH_URL }) {
...
var query = async function(options) {
  const { apiBaseUrl = SOURCE_DEFAULTS.apiBaseUrl, clientId = SOURCE_DEFAULTS.clientId, maxLengthURL = SOURCE_DEFAULTS.maxLengthURL, connectionName, sqlQuery, queryParameters } = options;
