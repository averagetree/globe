...
test.describe('Timeline DST', () => {
  test('it should handle DST start in UTC', async ({ page }) => {
    ...
    const days = await rulerEl.evaluate((el) => {
      var _a;
      const rulerSpans = (_a = el.shadowRoot) === null || _a === void 0 ? void 0 : _a.querySelectorAll('span');
      if (rulerSpans) {
...
test.describe('Timeline Interval Week', () => {
  ...
  test('it should show first of each week - cross year boundaries', async ({ page, }) => {
    const template = `
            <rux-timeline
                timezone="UTC"
...
test.describe('Timeline', () => {
  ...
  test('when an event is currently in the timelines range if the timelines range is changed so that the event becomes out of range should not display as partial and be hidden', async ({ page, }) => {
    const el = await page.locator('rux-timeline');
    await el.evaluate((e) => {
    ...
    await expect(timeRegion).not.toHaveClass('rux-time-region--partial-start rux-time-region--partial-end');
    await timeRegion
      .evaluate((e) => e.hasAttribute('partial'))
      .then((e) => expect(e).toBeFalsy());
  });
  test('if new event start date is before the timeline start date it should display as a partial start event', async ({ page, }) => {
    const el = await page
      .locator('rux-time-region[data-test-id="addEvent"] div')
      .first();
    const button = await page.locator('#add-partial-start-button');
    await button.click();
    await expect(el).toHaveClass('rux-time-region rux-time-region--partial-start');
  });
  test('if new event start date is after the timeline end date it should display as a partial end event', async ({ page, }) => {
    const el = await page
      .locator('rux-time-region[data-test-id="addEvent"] div')
      .first();
    const button = await page.locator('#add-partial-end-button');
    await button.click();
    await expect(el).toHaveClass('rux-time-region rux-time-region--partial-end');
  });
  test('if new event start date is before the timeline start date and the end date is after the timelines start date it should display an ongoing partial event', async ({ page, }) => {
    const el = await page
      .locator('rux-time-region[data-test-id="addEvent"] div')
      .first();
    const button = await page.locator('#add-partial-ongoing-button');
    await button.click();
    await expect(el).toHaveClass('rux-time-region rux-time-region--partial-start rux-time-region--partial-end');
  });
  test('existing in-range event is edited, if the event start date is before the timeline start date it should display as a partial start event', async ({ page, }) => {
    const el = await page
  ...
  test('existing in-range event is edited, if the event end date is after the timeline end date it should display as a partial end event', async ({ page, }) => {
    const el = await page
      .locator('rux-time-region[data-test-id="editEvent"] div')
      .first();
