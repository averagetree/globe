...
.rux-tab {
  ...
  border-bottom: var(--border-width-lg, 4px) solid transparent;
  cursor: pointer;
}
.rux-tab:hover {
...
.rux-tab--small {
  ...
  height: calc(var(--spacing-16, 4rem) - var(--spacing-1, 0.25rem));
  min-width: 5rem;
}

.rux-tab--large.rux-tab--selected {
  border-color: var(--color-border-interactive-default, #4dacff);
}

.rux-tab--small.rux-tab--selected {
  border-bottom: var(--border-width-lg, 4px) solid var(--color-border-interactive-default, #4dacff);
}
