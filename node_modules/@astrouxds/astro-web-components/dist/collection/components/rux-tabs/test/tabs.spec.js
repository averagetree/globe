...
test.describe('Tabs', () => {
  ...
  test('it can properly show tab content after being removed/added', async ({ page, }) => {
    // Add New Panel
    await page.evaluate(() => {
      ...
      tabsEl === null || tabsEl === void 0 ? void 0 : tabsEl.appendChild(newTabEl);
      tabPanelsEl === null || tabPanelsEl === void 0 ? void 0 : tabPanelsEl.appendChild(newTabPanel);
    });
    ...
    await page.evaluate(() => {
      ...
      newTabEl === null || newTabEl === void 0 ? void 0 : newTabEl.remove();
      const newTabPanel = document.getElementById('tab-panel-4');
      newTabPanel === null || newTabPanel === void 0 ? void 0 : newTabPanel.remove();
    });
    // Add Panel Back
    await page.evaluate(() => {
      const newTabEl = document.createElement('rux-tab');
  ...
  test('does not select tab if actions slot is clicked on', async ({ page, }) => {
    //Arrange
  ...
  test('does not emit ruxselected event when actions slot is clicked', async ({ page, }) => {
    //Arrange
    const tab3 = page.locator('#tab-id-3');
    const tab3Actions = tab3.locator('*[slot="actions"]');
  ...
  test('shows correct panel when its tab is clicked', async ({ page }) => {
    ...
    await ruxTabPanel2.evaluate((page) => {
      return (page.classList.contains('hydrated') &&
...
test.describe('Multiple tabs on same page', () => {
  ...
  test('it should have the first tab of each rux-tabs visible', async ({ page, }) => {
  ...
  test('it should not hide other rux-tabs content when a different rux-tab is clicked', async ({ page, }) => {
    const toClick = page
      .locator('rux-tabs')
      .first()
      .locator('rux-tab')
  ...
  test('it can dynamically add tabs that behave correctly', async ({ page, }) => {
    const template = `
            <rux-tabs id="tab-set-id-1">
                <rux-tab id="tab-id-1">Tab 1 title</rux-tab>
                <rux-tab id="tab-id-2">Tab 2 title</rux-tab>
...
test.describe('Tabs only Tab Keyboard Navigation', () => {
  ...
  test('it tabs into the first tab when none are selected', async ({ page, }) => {
    //Arrange
