...
test.describe('Input with form', () => {
  ...
  test('prepends aesthetics to label if required', async ({ page }) => {
    ...
    const ruxInputAesthetics = ruxInputComponent.locator('.rux-input-label .rux-input-label__asterisk');
    //Assert
  ...
  test('removes rux-icon if type is no longer password', async ({ page }) => {
    ...
    //Assert
    await expect(ruxInputIcon).toBeVisible();
    await ruxInputComponent.evaluate((e) => (e.type = 'text'));
  ...
  test('changes icon when icon is clicked', async ({ page }) => {
    ...
    //Arrange
    const ruxInputIconVisibilityOff = ruxInputIcon.locator('rux-icon-visibility-off');
    //Assert
    await expect(ruxInputIconVisibilityOff).toBeVisible();
  });
  test('cannot have its value changed if readonly is true', async ({ page, }) => {
    //Arrange
  ...
  test('changes autocomplete to off if type is password', async ({ page, }) => {
    //Arrange
  ...
  test('submits the correct value in type datetime-local', async ({ page, }) => {
    //Arrange
...
test.describe('Input emits correct events', () => {
  ...
  test('it emits ruxchange event when input is committed', async ({ page, }) => {
...
test('getInput method returns the internal input', async ({ page }) => {
  ...
  let test = await ruxInput.evaluate(async (el) => (await el.getInput()).classList);
...
test.describe('Min and max length', () => {
  test('minlength works', async ({ page }) => {
    ...
    await input.type('12');
    // Check validity
    await input.evaluate((el) => el.reportValidity());
    const validityState = await input.evaluate((el) => el.validity.tooShort);
    // Assert the validity
