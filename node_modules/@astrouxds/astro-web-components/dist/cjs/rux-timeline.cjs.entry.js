...
const RuxTimeline = class {
  ...
  _calculatePlayheadFromTime(time) {
    ...
    let newTime = Math.abs(helpers.differenceInMinutes(useStartEndDates.timelineStart, timeAsDate));
    if (this.interval === 'day') {
      newTime = Math.abs(helpers.differenceInHours(useStartEndDates.timelineStart, timeAsDate));
    }
    if (this.interval === 'week') {
      newTime =
        Math.abs(helpers.differenceInHours(useStartEndDates.timelineStart, timeAsDate)) / 7;
    }
    if (this.interval === 'month') {
      ...
      const monthStart = helpers.getBeginningOfMonth(useStartEndDates.timelineStart, 0);
      // number of months from start + 1/nth of the current month
      const numMonths = Math.abs(helpers.differenceInMonths(monthStart, timeAsDate));
  ...
  render() {
    return (index$1.h(index$1.Host, null, index$1.h("div", { class: "rux-timeline", onMouseMove: (ev) => this._handleMouse(ev), onScroll: () => this._handleScroll(), ref: (el) => (this.timelineContainer = el), part: "time-region-container" }, this.playhead && (index$1.h("div", { class: {
        'rux-playhead': true,
        hidden: !this.showPlayhead,
      }, part: "playhead", style: {
        top: `${this.playheadHeight}px`,
        left: `${this.playheadPositionInPixels}px`,
      } })), index$1.h("div", { class: "events", ref: (el) => (this.slotContainer = el) }, index$1.h("slot", { onSlotchange: this._handleSlotChange })), index$1.h("div", { class: "ruler", ref: (el) => (this.rulerContainer = el) }, index$1.h("slot", { name: "ruler" })))));
  }
